...
- [x] Add a Bluetooth LE proximity transport carrying the same signed RTP/1 payload, so two iPhones can pay each other by being held together — the one pair NFC can never serve, because iOS grants no third-party card emulation. Native on both platforms is peripheral and central at once, frames the payload to the negotiated MTU, and offers only a merchant above roughly -55 dBm.
- [x] Keep the radios honest about intent: a Bluetooth run of three readings at -45 dBm or better stands in for a tap and raises the device prompt with no button, a weaker run opens confirmation and waits for Approve, and a merchant publishing over both radios is paid once whichever arrives first.
- [x] Make the authorization policy a property of how the request arrived rather than of which transport carried it, so `useAutomaticAuthorization` takes an explicit decision instead of inferring one from `transport`.
- [x] Read the ledger a tap is checked against on arrival rather than on a timer. The root listener is armed for as long as the app is open, so polling there was a network request every five seconds all day to hold a number that was re-read anyway the moment a merchant turned up.
- [x] Restore automatic authorization for a tap taken from the scan screen, which is the only way to take one on iOS; the policy moved from `transport` to an explicit decision and this path was left behind.
- [x] Report only a real Core Bluetooth grant as authorized. Treating "not asked yet" as authorized hid the prompt, and the prompt is what allocates the managers that make the radio knowable, so iOS could never reach a state where it scanned.
- [x] Say one thing on the merchant counter about being approached instead of scanned, and say it about Bluetooth first — a merchant whose NFC is serving Android taps would otherwise learn from the first iPhone customer that half the counter was never switched on.
- [x] Stop a radio from taking the screen back. A customer who declines a request is still standing in range of it, so a request already shown is not re-offered over Bluetooth for thirty seconds; a tap always is.
- [x] Read the camera module by the name the native side registers. `react-native-camera-kit` exports `NativeModules.CameraKit` and registers `RNCameraKitModule`, so every permission call threw and the screen reported a phone with a camera as having none, with no prompt ever shown. Its own types export as `any`, and the test mocked the library's export, so both were green while the device was dead.
- [x] Stop treating `inactive` as the app having gone away. iOS reports it for any system UI over the app — including the Core NFC reader sheet, which *is* the feature: arming the reader presented the sheet, the sheet resigned the app active, and the reader was torn down about a second later by its own success. Same fix keeps the Bluetooth scanner, merchant HCE and the pending authorization alive behind a permission or biometric prompt.
- [x] Stop requiring a reference on a payment request. Most counters have nothing to say beyond the amount, and a required field with nothing true in it is answered with a keystroke; an empty one now carries the amount being asked for, which keeps the signed RTP/1 field non-empty and says something a customer can check.
- [x] Make the merchant's recent payments answerable. Rows were plain views, so tapping one did nothing; they open the request's detail now — amount, status, ledger, transaction and an explorer link — and a request that is over is dimmed and drops the plus sign rather than reading as money received.
- [x] Repoint installs that saved a development API address before this build shipped a hosted one. `apiBaseUrl` is persisted, no migration ever touched it, and on iOS the store lives in the Keychain — which survives deleting the app. A phone left on `http://127.0.0.1:4100` reaches nothing: no request publishes, no QR appears, nothing settles, and none of it says so. Version 7 repoints it once, and Developer Settings offers the way back without reinstalling.
- [x] Let a customer who onboarded with twelve words pay. Claiming a request is an authenticated call and the paying flow never established a session there, so the claim went out bare; and `assertWalletOwnership` compared a classic account against a smart wallet this API had never provisioned, so even a session would have been refused. Both ends fixed, and every countersignature failure now reaches the screen with the sentence it was raised with instead of one flattened line.
- [x] Put the cause of an unmapped settlement failure on screen. A phone has no console, so "the payment could not be completed" was the end of the investigation for whoever was holding it.
- [x] Drop an expired request from the counter instead of letting it hold the active slot. The home screen kept offering to open a dead request and the only way to take the next payment was to find "New request" underneath it.
- [x] Put a counter on the air over every transport at once. Creating a request raises the Bluetooth permission itself, so QR and Bluetooth are both live the moment the code is on screen; the payer is asked the same way when they open the scan screen. Neither ask blocks the QR, and a refusal leaves the manual control.
- [ ] Verify the Bluetooth transport on two physical phones, including an iPhone-to-iPhone pair, and tune the proximity threshold against real handsets.
- [x] Put the ISO-7816 AID list back in `Info.plist`, where Core NFC actually reads it, and leave only the `formats` entitlement in `RosaPay.entitlements`; an AID in the entitlements file is invisible to the reader and can fail signing.
- [x] Offer tapping from the home screen on iOS, where a Core NFC session cannot be armed silently, instead of only from inside the scanner.
