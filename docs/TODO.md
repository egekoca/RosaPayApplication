...
- [~] Add PostgreSQL migrations and a durable repository adapter; `buildApp` now accepts the adapter and intent creation has an atomic repository boundary, while production `pg` pool wiring remains.
...
- [~] Read `PaymentSettled` events through the generated contract spec and reconcile them against submitted API state only after a matching RPC receipt; worker cursor persistence now has a PostgreSQL adapter, while runtime wiring and durable event indexing remain.
...
2. Wire a transaction-capable `pg` pool into the API bootstrap, then add auth/capability checks and a scheduled worker loop around the confirmation port.
