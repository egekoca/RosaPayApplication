...
export function PaymentConfirmationScreen({route, navigation}: Props) {
  ...
  const mutation = useMutation({
    mutationFn: () => {
      setStage(undefined);
      return settlePaymentIntent(payload, {onProgress: progress => setStage(progress.stage)});
    },
    onSuccess: receipt => {
      addReceipt(receipt);
  ...
  return (
    <Screen>
      ...
      <Pulse active={mutation.isPending} style={styles.security}><ShieldCheck color={blocked ? colors.inkMuted : colors.success} size={18} /><Text style={[styles.securityText, blocked && styles.securityTextBlocked]}>{blocked ? 'This request cannot be authorized.' : 'Your device will authorize this exact amount.'}</Text></Pulse>
